		...
		<table>
			...
			<c:forEach items="${sessionScope.cart.items}" var="entry">
				<tr>
					...
					<td><input type="number" value="${entry.value.count}" min="1" max="8" onclick="changeCount('${entry.value.id}',this)"></td>
					<td>${entry.value.price}</td>
					<td>${entry.value.totalPrice}</td>
					<td><a href="javascript:void(0)" onclick="mydelete('${entry.value.id}')">删除</a></td>
				</tr>
			</c:forEach>
